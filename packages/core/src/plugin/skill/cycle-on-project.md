...
7. **Unattended autonomy does not expand authorization.** Explicit user action, physical interaction, account access, external communication, deployment, publication, and destructive repository/filesystem operations remain hard boundaries. Do not control a user's application or account merely because a technical path exists. Before delete-sync, recursive cleanup, worktree/history replacement, or generated-artifact removal, require an already-established workflow or explicit authorization, run a read-only preview, and prove the recovery source. After the main objective is complete, only accept lower-risk follow-up work.
