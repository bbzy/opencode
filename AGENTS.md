...
- The cycle prompt message is admitted only after the session drain is acquired (`loopRun` creates the user message inside `state.tryRun`), so a round coalesced by `SessionBusyError` leaves no stray `[Cycle #N]` prompt and consumes no round number. Round prompts carry process-local cross-round context: the previous round's response tail (`LoopState.lastRoundResult`), the current idle (`consecutiveDry`) status, the unfinished-todos line (fed from the session todo list when any item is pending/in-progress), a duplicate-delivery instruction, and a DONE exit gated on clean todos + verified fixes + an exhausted Plan phase — keep those lines intact when changing `Loop.buildCyclePrompt`.
